...
fromparse2onco <- function(path_to_parse,tumor_only = FALSE,
                           filter_column=c("PASS"), VAF_tumor=0, VAF_control=0, total_tumor_reads=0, alt_tumor_reads=0, cgi=FALSE, oncokb=FALSE, cgi_list=c("oncogenic (predicted)", "oncogenic (predicted and annotated)", "oncogenic (annotated)"), oncokb_list=c("Likely Oncogenic", "Oncogenic"), annott=c("HIGH","MODERATE","MODIFIER"),
                           remove=FALSE, flags=FALSE, minimalMutations = 2, topgenes = 20000, nonSyn=c("Frame_Shift_Del", "Frame_Shift_Ins", "Splice_Site", "Translation_Start_Site","Nonsense_Mutation", "Nonstop_Mutation", "In_Frame_Del","In_Frame_Ins", "Missense_Mutation"),
                           Missense_color="#2a9134", Nonsense_color="#ffca3a", Nonstop_color="#000000", FrameDel_color="blue", FrameIns_color="purple", In_Frame_Ins_color="lightblue", In_Frame_Del_color="plum1", Tranlsation_Start_Site_color="#ff0a54", Splice_site_color="darkorange", Multihit_color="#dab49d")
                           {
  
  # Read parse_variants output and convert to MAF
  variants_df <- fromparse2table(path_to_parse = path_to_parse, tumor_only, oncokb, cgi)
  
  # Filter MAF data frame
  filtered_df <- filtering_table(variants_df, tumor_only, filter_column, VAF_tumor, VAF_control, total_tumor_reads, alt_tumor_reads, cgi, oncokb, cgi_list, oncokb_list, annott)
  
  # Create oncomatrix from MAF data frame
  modclassvartmb(filtered_df, remove, flags, minimalMutations, topgenes, nonSyn)
  
  # Make oncoplot
  oncoplot <- confoplot(Missense_color, Nonsense_color, Nonstop_color, FrameDel_color, FrameIns_color, In_Frame_Ins_color, In_Frame_Del_color, Tranlsation_Start_Site_color, Splice_site_color, Multihit_color)
